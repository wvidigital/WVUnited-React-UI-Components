...
import {textStyles, errorStyles} from '../../../stories/theme';
...
export default function FieldSelect(props) {
  ...
  /**
   * Renders the select field validation errors.
   */
  ...
  `;
    ...
    return (
      <FieldSelectContainer
        className={`form__item form__item--select form__item-${props.name} form__item--required-${props.fieldState.required} ${errors.hasErrors}`}>
        <label htmlFor={name}>
          <span className="label__text">{props.field.label}</span> {errors.messages}
        </label>
        <FieldSelect
          name={props.name}
